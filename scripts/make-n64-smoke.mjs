#!/usr/bin/env node
// Build the CC0 "LWX N64 Smoke Test" libdragon homebrew ROM from
// games/n64-smoke/.
//
// Unlike the other make-*.mjs scripts (native Windows toolchains), this one
// builds with libdragon via the anacierdem/libdragon Docker image, run from
// WSL2 — same environment used to build the mupen64plus_next core itself
// (see docs/N64_CORE_BUILD.md).
//
// Steps: sync games/n64-smoke/ into a WSL2-native scratch directory (avoids
// /mnt/c/... Docker-volume friction), run `make` inside the container, copy
// the built .z64 back.
//
// Needs: WSL2 with a distro that has Docker available (default distro name
// "Ubuntu", override with WSL_DISTRO) and the anacierdem/libdragon image
// pulled (`docker pull anacierdem/libdragon`).
// Usage: node scripts/make-n64-smoke.mjs
...
const GAME_DIR = resolve(__dirname, '..', 'games', 'n64-smoke');
const ROM_NAME = 'lwx-n64-smoke.z64';
const OUT = resolve(__dirname, '..', 'public', 'roms', 'freeware', ROM_NAME);

const WSL_DISTRO = process.env.WSL_DISTRO || 'Ubuntu';
const WSL_DIR = '~/n64-smoke-build';
