...
export const SYSTEMS = {
  ...
  //
  // FIXED 2026-07-27 (flipped scripts/probe-psx-guncon.js's gating "GUNCON
  // END-TO-END SIGNAL" check from FAIL to PASS: 12/13 → 13/13; that probe
  // carries 14 assertions since 2026-07-29 and is green at 14/14. That gating
  // check was REPLACED on 2026-07-29 by "[GUNCON AIM DISCRIMINATION]": the old
  // one compared the post-shot frame against the long-ago boot baseline and
  // also passed with no gun connected at all, so it never discriminated; the
  // replacement fires the identical trigger sequence on-target vs off-screen
  // and is validated against a no-gun negative control). Root cause,
  // confirmed by reading the source rather than guessing: mednafen_psx_hw is a
  // bespoke from-scratch build (docs/PSX_CORE_BUILD.md) whose build tooling
  ...
  // now advances the game's own state machine, not just a pixel-diff metric
  // — see docs/PSX_CORE_BUILD.md's "Verified end-to-end" note for the
  // screenshot description (files are tmp/psx-guncon-*.png, gitignored,
  // regenerated by each probe:psx-guncon run). CAVEAT: that verification was
  // against the 2026-07-27 12:17 build, which was silently reverted on disk at
  ...
  psx:       { label: 'PlayStation',        defaultCore: 'mednafen_psx_hw',  cores: ['mednafen_psx_hw'],              exts: ['chd','cue','m3u','ccd','pbp','exe'], aliases: ['psx','ps1','playstation','sony playstation'], thumbnailRepo: 'Sony_-_PlayStation', medium: 'floppy', experimental: true,
    ...
    //
    // `broken: false` since 2026-07-29 — VERIFIED, see "How it was verified"
    // below. Two things had to be true, and only one of them was ever written
    // down while this was gated:
    //   1. the core artifact must export rwebinput_set_lightgun(port,x,y,
    //      buttons) (docs/patches/rwebinput-lightgun-multiport.diff), and
    //   2. the app must actually CALL it for this core.
    // (1) came back with the 2026-07-29 core restore. (2) was a second, unlogged
    // blocker: PSX is the app's ONLY worker-execution gun system (CORES.
    // mednafen_psx_hw.execution = 'worker'), and the multiport call site lived
    // exclusively in the MAIN-THREAD EmulatorClient.sendLightgun (which the SNES
    // Justifier rides and PSX never touches). The worker's
    // EmulatorWorkerRuntime.forwardLightgun() had NO multiport branch at all —
    // it used `port` only as a gunDownByPort trigger-edge key and pushed every
    // aim through the shared canvas mousemove, so two GunCons really would have
    // read one pointer, exactly as this comment warned, for app-side reasons
    // that would have survived any number of core rebuilds. That branch was
    // added (2026-07-29) as the worker twin of EmulatorClient's, and THAT is
    // what made this flag flippable.
    //
    // An earlier version of this comment said the shipped mednafen_psx_jit
    // build "has the BASE gun patch but NOT the multiport one". That was wrong
    // twice over. What actually happened: the 2026-07-27 12:17 rebuild carried
    // BOTH patches and passed `npm run probe:psx-guncon` 13/13, but at 12:36 the
    // same day public/cores/ was overwritten from an 09:44 pre-gun-work backup
    // — most likely collateral damage from rolling back a Lightrec/GL experiment
    // — so the artifact that then shipped carried NEITHER patch. public/cores/
    // is gitignored, so nothing appeared in `git status`, and the single-gun
    // `lightgun` block above stayed `broken: false` the whole time while the
    // core on disk could no longer register a shot. Restored 2026-07-29 by
    // copying the 12:17 build back out of the WSL build dir (scripts/cores/psx/
    // core-build/dist — identical upstream pins to the clobbered artifact, so a
    // straight regression, not a divergent experiment).
    //
    // Current state, measured not assumed: `grep -c rwebinput_set_lightgun
    // public/cores/mednafen_psx_jit_libretro.js` reads 1 (as does the _clear_
    // counterpart), matching snes9x/nestopia/play/mupen64plus_next; only the
    // .js glue is a valid tell (the .wasm minifies export names and the
    // .worker.js is pthread glue — both read 0 for EVERY core).
    // public/cores/PATCHED.json records mednafen_psx_jit as ["base","multiport"]
    // and scripts/test-patched-cores.mjs re-checks that against the artifact on
    // every `npm test`.
    //
    // HOW IT WAS VERIFIED (`npm run probe:psx-twogun`, 23/23, 2026-07-29):
    // against the REAL core and a REAL commercial disc (Time Crisis), booted
    // through the REAL cartridge-insert path with twoGun:true. Four independent
    // layers, each falsifying the shared-pointer failure mode:
    //   • SEATING   — the worker wrote device 260 on BOTH libretro ports
    //                 (inputDevices {1:260, 2:260}) into the RA cfg + remap.
    //   • ROUTING   — both ports' aims went through the per-port export
    //                 (path='multiport', gun.multiport=true), holding distinct
    //                 coordinates at the same instant: (77,246) vs (435,246).
    //   • RENDERING — Beetle PSX drew TWO crosshairs at once, in its per-port
    //                 colours (port 0 red [247,62,32], port 1 blue [56,158,223])
    //                 at the two guns' own positions on one identical
    //                 background; gun 1's cursor stayed put while gun 2 moved
    //                 and only left when gun 1 itself was moved. That is the
    //                 CORE reading back two independent gun states. See
    //                 tmp/psx-twogun-cursors-apart.png.
    //   • ISOLATION — the same shot at the same point differing ONLY in `port`
    //                 gave different outcomes: gun 2 firing at the calibration
    //                 target changed nothing (maxDiff=0 — that 1P screen polls
    //                 port 0), gun 1 firing at the identical point registered
    //                 (maxDiff=287, the same signal probe:psx-guncon's green
    //                 single-gun control produces). Under one shared pointer the
    //                 gun-2 shot would have registered too.
    // SCOPE: this is core-level + per-port isolation against a real game, NOT a
    // played-through 2-player co-op session — nobody has yet driven Point Blank
    // or Lethal Enforcers I & II through their 2P menus with two guns. That is
    // the remaining confidence gap; the mechanism itself is proven.
    // Full writeups: docs/LIGHTGUN_SUPPORT.md, docs/PSX_CORE_BUILD.md.
    lightgun2: { label: 'GunCon (2-gun)', core: 'mednafen_psx_hw', devices: [260, 260], ports: [0, 1], broken: false, coreOptions: {
      beetle_psx_gun_input_mode: 'lightgun', beetle_psx_hw_gun_input_mode: 'lightgun',
      beetle_psx_gun_cursor: 'cross', beetle_psx_hw_gun_cursor: 'cross',
    } } },
 ...
 * flagged `broken: true` reports false here, so real users are never
 * offered/auto-armed into a gun that provably can't register a shot — the same
 * gate lightgunLoadConfig enforces below. The flag is for guns whose app-side
 * wiring is believed right but that have been shown NOT to register a shot
 * end-to-end (it was last raised on PSX's GunCon, since lifted — see
 * SYSTEMS.psx.lightgun's comment for that history); no registered single-gun
 * descriptor carries it today, and scripts/test-systems.mjs injects a temporary
 * `__gatedtest` fixture to keep this path covered.
 */
 ...
 * means a `twoGun: true` game on a system whose two-gun device is gated still
 * gets its ONE working gun. No registered `lightgun2` carries `broken: true`
 * today (PSX's was un-gated 2026-07-29 by scripts/probe-psx-twogun.js), so that
 * fallback has no live example — scripts/test-systems.mjs injects a temporary
 * `__gatedtest` fixture to keep it covered. Probes that want a gated config ask
 ...
 *
 * `opts.allowBroken` bypasses the `broken:true` gate — for probes/de-risk
 * tooling ONLY: it exists so a gun that is still gated pending verification can
 * be driven through the exact app-side wiring a real boot would use, which is
 * how such a gate ever gets lifted. Every real caller (loadCartridge, the in-VR
 * gun-arm reboot path) omits it, so a gated gun is never silently offered to a
 * real user. Note the corollary: a probe that sets it proves nothing about the
 * gate's own state, so a probe guarding an UN-gated device should not set it
 * (scripts/probe-psx-twogun.js runs flag-free for exactly this reason).
 */
