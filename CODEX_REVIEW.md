...
| **ARC-4** | **Measured 2026-08-21, and the recommendation is refused as written.** `TestApi` costs **17.6 KB raw / ~6.8 KiB gzip** - 2% of the 309 KiB bundle - and it does not sit in the `main` chunk at all: it rides the shared chunk both entries import (splitting it out moved 17.7 KB off `Collection`, left `main` +41 bytes, and changed the total by -49 bytes). Against that, `window.__testApi` is what 11 verification scripts and `tmp/boot-check.mjs` drive, and this project's probes run against PRODUCTION builds - the headless boot check is what caught the critical `makeGamepadHoldKey` ReferenceError. The raw hooks the finding also names (`__forceCfgExtra` / `__forceExtraFiles`) are how the light-gun, remap and PS2 bring-ups were done (~10 scripts in `tmp/`). The security half of the argument is weak: a same-origin script compromise already has full capability, so the API adds convenience, not privilege. Trading the project's only real safety net for 2% of a cold download is a bad trade; revisit only if a headset measurement shows startup parse cost that matters. |
| **docs index** | No `docs/README.md` marking which files under `docs/` are current vs. historical (CLAUDE_REVIEW section 8). |
| **`docs/ROADMAP.md` loose end 19** | Four of the six negative-only "nothing happened" checks got a mutation-checked positive arm 2026-08-21; the two `probe-local-rom*` "no network request" checks still have none. (`test-routing:175-180` was already closed by `bb30ba8`'s vacuous-assertion audit - it asserts `r.length > 0` before the loop - and this row was stale.) |
